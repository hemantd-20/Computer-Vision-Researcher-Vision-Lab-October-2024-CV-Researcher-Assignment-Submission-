...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, masks, refpoint_embed, pos_embeds, tgt, attn_mask=None):
        ...
        # Begin Encoder
        #########################################################
        memory, enc_intermediate_output, enc_intermediate_refpoints, encoder_attention_weights = self.encoder(
                src_flatten, 
        ...
        # Begin Decoder
        #########################################################
        hs, references, decoder_attention_weights = self.decoder(
        ...
        return hs, references, hs_enc, ref_enc, init_box_proposal, encoder_attention_weights, decoder_attention_weights
        # hs: (n_dec, bs, nq, d_model)
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, 
        ...
        intermediate_output = []
        intermediate_ref = []
        attention_weights_all_layers = []  # Store attention weights
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            if not dropflag:
                if self.deformable_encoder:
                    output, attention_weights = layer(src=output, pos=pos, reference_points=reference_points, spatial_shapes=spatial_shapes, level_start_index=level_start_index, key_padding_mask=key_padding_mask)
                    attention_weights_all_layers.append(attention_weights)  # Collect attention weights
        ...
        return output, intermediate_output, intermediate_ref, attention_weights_all_layers
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, memory,
        ...
        # self_attention_weights_all_layers = []

        cross_attention_weights_all_layers = []
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            if self.dec_layer_dropout_prob is not None:
                ...
                if prob < self.dec_layer_dropout_prob[layer_id]:
                    dropflag = True
            if not dropflag:
                output, cross_attention_weights = layer(
                    tgt = output,
                ...
                cross_attention_weights_all_layers.append(cross_attention_weights)
        ...
        return [
            ...
            [weights for weights in cross_attention_weights_all_layers]
        ]
...
class DeformableTransformerEncoderLayer(nn.Module):
    ...
    #     return src
    def forward(self, src, pos, reference_points, spatial_shapes, level_start_index, key_padding_mask=None):
    # self-attention with modified return
...
class DeformableTransformerDecoderLayer(nn.Module):
    ...
    def forward_sa(self,
        ...
        if self.self_attn is not None:
            if self.decoder_sa_type == 'sa':
                ...
                tgt2, self_attention_weights = self.self_attn(self.with_pos_embed(tgt, tgt_query_pos).transpose(0, 1),
        ...
        return tgt, self_attention_weights
    ...
    def forward_ca(self,
        ...
        # Unpacking the output and cross_attention_weights from self.cross_attn
        tgt2, cross_attention_weights = self.cross_attn(self.with_pos_embed(tgt, tgt_query_pos).transpose(0, 1),
                               tgt_reference_points.transpose(0, 1).contiguous(),
                               memory.transpose(0, 1), memory_spatial_shapes, memory_level_start_index, memory_key_padding_mask)
        # tgt_reference_points = tgt_reference_points.view(1100, 2, 4)
        # tgt_reference_points = tgt_reference_points[..., 0]
        tgt2 = tgt2.transpose(0, 1)
        # print("&&&&&&&&&&&&&&&tgt shape:", tgt.shape)
        # print("tgt_reference_points shape:", tgt_reference_points.shape)
        # print("memory shape:", memory.shape)
        # print("memory_spatial_shapes shape:", memory_spatial_shapes.shape)
        # print("memory_level_start_index shape:", memory_level_start_index.shape)
        # print("cross_attention_weights shape:", cross_attention_weights.shape)
    ...
    def forward(self,
        ...
        for funcname in self.module_seq:
            if funcname == 'ffn':
                ...
                tgt, cross_attention_weights = self.forward_ca(tgt, tgt_query_pos, tgt_query_sine_embed, \
                    tgt_key_padding_mask, tgt_reference_points, \
                        memory, memory_key_padding_mask, memory_level_start_index, \
                            memory_spatial_shapes, memory_pos, self_attn_mask, cross_attn_mask)
            elif funcname == 'sa':
                tgt, self_attention_weights = self.forward_sa(tgt, tgt_query_pos, tgt_query_sine_embed, \
                    tgt_key_padding_mask, tgt_reference_points, \
                        memory, memory_key_padding_mask, memory_level_start_index, \
                            memory_spatial_shapes, memory_pos, self_attn_mask, cross_attn_mask)
            else:
